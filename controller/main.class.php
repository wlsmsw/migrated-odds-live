...
class Site extends Config
{
	...
	/*
	 * @dec: get query string from URL 'sport=tennis'
	 */
	function get_url()
	{
		if (isset($_GET['sport']))
		 	return $this->request = "sport=" . $_GET['sport'];
		else if (isset($_POST['sport']))	
			return $this->request = $_POST['sport'];
		else
			return "";
	}
	...
	function check_request()
	{
		...
		// will not accept any parameter except 'sport'	
	...
	/**
	 * @desc: get value of parameter
	 */
	function get_request_value()
	{		
		$r_value = array_values($this->query);

		if (isset($r_value[0]))
			$this->request_value = $r_value[0];
		else
			$this->request_value = "";
	}
