  ...
  function updateSet(match, data) {
    ...
    for (i=0; i<5; i++)
    {
      if(typeof(data.set_details[i]) != "undefined" && data.set_details[i] !== null) 
      {
		...
		//console.log('setting set detail of ' + data.players.A + ' set: ' + data.set_details[i].a);
		//console.log('setting set detail of ' + data.players.B + ' set: ' + data.set_details[i].b);
  ...
  function updateMatch(match, data) {
    ...
    {  
      for (i=0; i<5; i++)
      {
        ...
        {   
		  //console.log('setting match detail of ' + data.players.A + ' set: ' + data.match_detail[i].a);
		  //console.log('setting match detail of ' + data.players.B + ' set: ' + data.match_detail[i].b);
  ...
  function updateOdds(match, data) {
    ...
    if(typeof(data.odds_details.players_odds) != "undefined" && data.odds_details.players_odds !== null) 
    {
      ...
      $.each(data.odds_details.players_odds, function(odds_name, odds_value) 
      {
        ...
        // DRAW CLASS
        //console.log('odds draw marker: ' + odds_draw_marker); 
        if (odds_draw_marker == 1)
        {
           $('#odds_div_' + match).addClass('odds-selection draw');  
           //console.log('adding class draw');
        }
        else
        {
  ...
  function removeSports() {
    ...
    //console.log('cleaning inactive sports.................');
    var sportsIDs = [];
    $(".content-wrapper").find(".sports-ul").each(function(){ sportsIDs.push(this.id); });
    
    var inactive_sport = diff(sportsIDs, myids.sports); 
    
    //console.log('posted sports: ' + sportsIDs); // processed sports id
    //console.log('new sports: ' + myids.sports); // new sports id data
    //console.log('diff sport: ' + inactive_sport); // id's left
  ...
  function removeLeagues() {
    ...
    //console.log('cleaning inactive leagues...................');
    var leaguesIDs = [];
    $(".content-wrapper").find(".ul-league").each(function(){ leaguesIDs.push(this.id); });
    //console.log('posted leagues ids: ' + leaguesIDs); // processed leagues id
    ...
    //console.log('new league ids: ' + tmp_league); // new leagues id data
    //console.log('diff leagues: ' + inactive_leagues); // id's left
       
    $.each(inactive_leagues, function(index, unleague) {
      //console.log('XXX removing league: ' + unleague);
     
      lgid = $("#"+unleague).parent().attr("id");
      //console.log('XXX removing the parent div of league: ' + lgid);  
      $("#"+lgid).remove();
    });
  ...
  function removeMatches() {
    ...
    //console.log('cleaning inactive matches...................');
    var matchesIDs = [];
    ...
    //console.log('posted match ids: ' + matchesIDs); // processed sports id
    //console.log('new match ids: ' + myids.match); // new sports id data
  ...
  function appendLeague(league_ctr, league_id, sport_ctr, match_id, sport_id) {
    ...
    { 
      //$("#end-page").remove();   // remove end page first
  ...
  $.each(league, function(league_name, lval) 
  {
    $.each(lval, function(match, data) 
    {
      ...
      { 
		...
		//console.log('coming in.. updating scores in aplayer_' + match + ' value: ' + data.score_detail.a + ' name: ' + data.players.A);
		//console.log('coming in.. updating scores in bplayer_' + match + ' value: ' + data.score_detail.b + ' name: ' + data.players.B);
		...
		//$(".scores-container").find("#aplayer_" + match).html('xxx');
        //$(".scores-container").find("#bplayer_" + match).html('yyyy');
...
/*
window.setInterval("reloadIFrame();", 900000); // 15 minutes 900000
