...
class MobileAPI extends reqAPI{
	...
	function sortLiveData()
	{
		...
		if (empty($tmp)) return "tmp is false";;
		$tmp = null;
		...
		$num_ids 	 = (count($ids))-1;
		if (!isset($json_result->items)) return "json items not set";
		...
		else
		{
			...
			for ($i=0; $i<=$num_ids; $i++)
			{
				...
				if (array_key_exists('l'.$ids[$i], $items))
				{
					...
					$betradar_mid		= (isset($valid_key->brMatchId)) ? $valid_key->brMatchId : ""; 
					...
					$live_data[$sport_code . '-l' . $ids[$i]] 
													= array(
														...
														"betradar_mid"	 => $betradar_mid,
														"category"		 => $category,
														"country"		 => $country,
														"country_league" => $country . " : " . $league,
														"players"		 => array(
																			'A' => $playerA,
																			'B'	=> $playerB,
																			),
	...
	*/
		...
		/**
	 * @param: none
	 * @return: ARRAY if success otherwise STR Error Message
	 */

	function sortLiveData()
	{
		// $this->result = the JSON result from cURLing the API
		...
		else
		{
			for ($i=0; $i<=$num_ids; $i++)
			{
				...
				if (isset($ids[$i])) 
				{
					$currentID = substr($ids[$i], 1);

					// get live detail
					if (array_key_exists('l'.$currentID, $items))
					{
						$valid_key 			= $items['l'.$currentID];
                        ...
                        if(strpos($leaguename, 'electronic') !== false) {
						    ...
						    $live_data[$sport_code . '-l' . $currentID] 
														= array(
															"sport"			 => $sport,
															"sport_code"	 => $sport_code,
															"description" 	 => $game_description,
					...
					// get match detail
					if (array_key_exists('m'.$currentID, $items))
					{
						$valid_key 		= $items['m'.$currentID];
						$live_parent 	= (isset($valid_key->parent)) ? $valid_key->parent : "";
						...
						foreach($live_data as $k => $v)
						{
							...
							if ($live_key == $live_parent)
							{
								...
								$live_data[$arrk[0]."-".$live_key]['odds_id'] = 'm'.$currentID;
								$live_data[$arrk[0]."-".$live_key]['m'.$currentID] 
										= array(
											"desc" 			=> $match_desc,
											"pos"	 		=> $match_pos,
											"market_type"	=> $match_market_type,
					...
					// get odds detail
					if (array_key_exists('o'.$currentID, $items))
					{
						$valid_key = $items['o'.$currentID];		
						$parent_id = (isset($valid_key->parent)) ? $valid_key->parent : "";
